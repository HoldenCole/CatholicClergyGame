            ...
            return (
              <li key={project.type} className="flex flex-wrap items-baseline justify-between gap-2">
                <span>
                  ...
                  {project.stalledWeeks > 0 ? `, stalled ${project.stalledWeeks} weeks for want of money` : ''}. Worked in its own time, beside the week
            ...
            <li key={def.type}>
              <button disabled={!available} onClick={() => { start(def.type); setPicking(false); }} className="choice">
                <div className="text-sm">
                  {def.label} <span className="ink-faint text-xs">· {Math.round((def.weeks / 52) * 10) / 10} years, beside the week{def.cost ? `, $${def.cost.toLocaleString()}` : ''}</span>
                </div>
