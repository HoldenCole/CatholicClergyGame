    ...
    <header className="plate hud flex items-center justify-between gap-6 px-5 py-2">
      <div className="flex min-w-0 items-center gap-4">
        ...
        <span className="truncate text-sm">
          {c ? `${c.name.first} ${c.name.last}, ` : ''}{game.study ? `Studying in ${game.study.city === 'rome' ? 'Rome' : 'Washington'}, year ${Math.floor((clock.week - game.study.startWeek) / 52) + 1}` : (PHASE_LABELS[game.phase] ?? game.phase)}
        </span>
