...
export default function StudyRoutinePanel() {
  ...
  const where = study.city === 'rome' ? 'Rome' : 'Washington';
  ...
  return (
    <>
      <Sheet title={`${study.label} in ${where}`}>
        <p className="ink-muted text-xs leading-relaxed">
          {study.school.charAt(0).toUpperCase() + study.school.slice(1)}, living at {study.residence}. {Math.floor(weeksLeft / 52) > 0 ? `${Math.floor(weeksLeft / 52)} years and ` : ''}{weeksLeft % 52} weeks until the degree{study.failed ? ', if it comes' : ''}. Lectures, the chapel, and the house rule take the week; {budget} hours are yours.
      ...
      <Sheet title="Your studies">
        <ul className="flex flex-col gap-1.5">{studies.map(row)}</ul>
        <p className="ink-faint mt-2 text-xs">The thesis is the degree. Everything else is what kind of priest comes home.</p>
      </Sheet>
