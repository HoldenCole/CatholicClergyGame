...
**Houses that live** (`systems/religious/growth.ts`). No house is generated tiny unless it has just been founded: each kind has a floor (`PROVINCE.floor`) and a priory keeps ten men or more. Every year every house of the province draws men on its kind, its works, its observance and cohesion, what it has built, and its prior's name (the player's reputations, an NPC's presence), less as it fills; the men go to the novitiate and come home professed, credited to the house. The old die by age and the young leave by vows; a dead prior is replaced by the eldest solemnly professed priest until the chapter. Each work brings the house money a year and each man costs it; a house in the red is carried by the province, which remembers whose it was. **The prior builds** (`content/religious/priorDesk.json` builds): one project at a time from the purse, the province askable for half once a year; a restored chapel, a library wing, a guest wing, an enlarged church, classrooms (which give the house teaching), a wing for the studium or the novitiate (beds), and a school of the order's own. Each stands when its weeks are up, draws vocations, and some bring income or a work. A vocations weekend from the purse lifts the draw for a year. The player's line of foundations keeps its own model (§9.5).

**The vote on a profession** (`systems/religious/formation.ts`, `VOWS`) is the house's regard with some noise in it, not a lottery: each professed man's chance of a yes starts high (a house professes the men it has formed unless it has a reason), moves with his own regard for the man, the man's standing with the house, and the novice master's or master of students' word, and falls with this year's concerns only, never the old years'. A house that holds a man back writes no concern, and the next vote is a shade easier, since the house has watched him a year longer.

**Letters by campaign.** An offer carries `campaign`: the diocese's letters (absent) never reach a friar, and the order's (`religious`, `content/offers/religious.json`) never reach a priest of the diocese; `engine/offers.ts` reads it. The provincial's letter about Rome (`fr_rome_study`, the order's own faculty, unnamed) moves him the week he says yes, since it is the order's own; the study years run as the base game's, and he comes home to a consultation, not the board. **A short-handed house leaves the work to him**: a house whose need is high, or with fewer men than twice its works, offers its works in the consultation (`ConsultationOption.works`), and the provincial honours the work he asked for when he sends him there.

**The rite of the order** (Dominicans). A student brother learns it in the crypt: the chapel activity `dominican_rite` or the students' rite society, either of which earns the credential `dominican_rite` and the flag `can_celebrate_dominican_rite` after a year of hours; the scenes in `events/religious/dominican_rite.json` teach and test it through formation (the first Mass seen, the Salve, serving it, the museum argument, the master of students' question, the deacon's Gospel) and after ordination (the first Mass in the rite, the lay Dominicans asking for it). Ordained, it is a spend (`order_rite`, gated on the credential) that builds the liturgist's name and the observant bloc's regard. The rite's details are kept to what is well attested and flagged for verification in the content.
...
**QA pass against the diocesan game** *(added at build time)*. Measured over a year at several points of a career, a friar's week read thinner than a priest's: fewer lines, more repeats, no letters, fewer scenes, and screens that still spoke of the seminary, the rector, and the parish. Fixed as data or on existing systems: the friar's rooms (a cell and a cloister, with hotspots bound to his week's spends and the house's sheets), the novice master and master of students in the evaluation and the briefing, the Record's lanes in the house's words, the men of his class as brothers of the order with brothers' milestones, a studium in every province, the order's default week at ordination (`OrderDef.mechanics.defaultSpends`), the hours of accepted commitments counted in the friar's load, the order's own letters (`content/offers/religious.json`, formation and priest), a line of the house most weeks (`content/religious/houseLife.json`), and scenes for the player as prior (`shared_prior.json`), since the scenes that speak of the prior as another man are closed to him. The scene rate for a friar is 0.18 a week (invented).

**QA pass, round two** *(added at build time)*. Fixed: an office laid down early is vacated properly (`systems/religious/vacate.ts`): a prior elected provincial or founding a house leaves the priorship, its flag, and the house's chair, which goes to the house's eldest solemnly professed priest until it elects; a closed house takes its prior's office with it and moves the player with its men. A charter draft opens on options the diocese allows (`settleDraft`). A novice's free hours begin as the order's (`OrderDef.mechanics.defaultFormationRoutine`). The Record's week of spends does not repeat the last two weeks. Dominican flavour as data, gated on `order:OP`: six more days of the order's calendar with scenes (`dominican_feasts2.json`), the order's own customs in the week (`houseLife.json`, pool `order:<key>`), nine letters (the Rosary in October, the public disputation, Preacher General, promoter of the Rosary, the nuns' chaplaincy, the Lenten course, the summer institute on St. Thomas, diffinitor to the general chapter, the Holy Name Society), sixteen career scenes (`dominican_career5.json`) weighted to the later years and to the player as provincial, and the order's motto framed in the cell (`OrderDef.mottoShort`). Order customs and titles are flagged for verification.
