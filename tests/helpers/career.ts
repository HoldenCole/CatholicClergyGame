...
/**
 * A friar's career through the store: the order and a province chosen, the
 * novitiate, the seven years, ordination into the provincial's consultation,
 * and the years after with their letters, chapters, and terms, every
 * decision taken the plain way. E3 round 1's done criterion, driven.
 */
export function playFriar(seed: string, order: 'OP' | 'OSA', maxWeeks: number, a: CreationAnswers = defaultAnswers, policy: Policy = 'first'): GameState {
  const s = useGameStore;
  s.getState().newGame({ seed, start: { year: 2010, month: 8, day: 20 }, campaign: 'religious' });
  s.getState().chooseOrder(order);
  const candidates = s.getState().game!.provinceCandidates ?? [];
  const province = candidates[policy === 'last' ? candidates.length - 1 : 0]!;
  s.getState().startReligious({ ...a, tie: 'transfer' }, { order, provinceId: province.id, why: 'charism', tie: 'outside', religiousName: 'Thomas' });
  for (let i = 0; i < 30000; i++) {
    ...
    if (game.offers.length) {
      const o = game.offers[0]!;
      s.getState().declineOffer(o.offerId);
      if (s.getState().game!.offers.length === game.offers.length) s.getState().markOfferRead(o.offerId);
      continue;
    }
    switch (game.mode.kind) {
      case 'year_start': {
      ...
      case 'ordination': s.getState().ordain(); break;
      case 'assignment': s.getState().acceptAssignment(); break;
      case 'assignment_choice': s.getState().chooseAssignment(game.mode.options[0]!.id); break;
      case 'letter': s.getState().readLetter(); break;
      case 'director': s.getState().chooseDirector(game.mode.options[0]!.npcId, true); break;
      case 'consultation': s.getState().letProvincialDecide(); break;
      case 'obedience_letter': s.getState().answerLetter('good'); break;
      case 'chapter':
        if (!game.religious?.chapter?.outcome) s.getState().holdBallots();
      ...
      case 'charter': s.getState().writeCharter(); break;
      case 'clock':
        s.getState().setSpeed('SKIP');
        s.getState().runToStop();
        break;
